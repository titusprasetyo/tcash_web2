...
%>
        ...
        <table width="21%" border="0" cellspacing="0" cellpadding="0">
          <form name="form" method="post" action="deposit_approval.jsp">
            <tr bgcolor="#CC6633"> 
...
%>
        ...
        <table width="80%" border="1" cellspacing="0" cellpadding="0" bordercolor="#FFF6EF">
          ...
          <tr> 
            ...
            <td bgcolor="#CC6633"><div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><strong>Telkomsel Bank</strong></font></div></td>
            </tr>
		  <%
...
// end of paging stuff
		 	...
		 	if (request.getParameter("merchant_id") != null && !(request.getParameter("merchant_id").trim()).equals("")) {
		 		sql = "select * FROM (select a.*, ROWNUM rnum from (" + 
		 					"select a.DEPOSIT_ID, a.IS_EXECUTED, a.DEPOSIT_TIME, c.NAME, c.TSEL_BANK_ACC, a.DOC_NUMBER, a.NOTE, c.BANK_NAME, c.BANK_ACC_NO, d.MERCHANT_ID, d.ACC_NO, d.SETTLE_DATE, d.AMOUNT, d.STATUS from MERCHANT_DEPOSIT a, MERCHANT b, MERCHANT_INFO c, SETTLEMENT_HISTORY d where a.MERCHANT_ID=b.MERCHANT_ID and b.MERCHANT_INFO_ID=c.MERCHANT_INFO_ID and a.MERCHANT_ID='"+request.getParameter("merchant_id")+"' and IS_EXECUTED='0' and d.STATUS='1' order by DEPOSIT_TIME DESC" +
		 					" ) a where ROWNUM <= ?) where rnum >= ?";
		 	} else {
				sql = "select * FROM (select a.*, ROWNUM rnum from (" + 
							"select a.DEPOSIT_ID, a.IS_EXECUTED, a.DEPOSIT_TIME, c.NAME, c.TSEL_BANK_ACC, a.DOC_NUMBER, a.NOTE from MERCHANT_DEPOSIT a, MERCHANT b, MERCHANT_INFO c, SETTLEMENT_HISTORY d where a.MERCHANT_ID=b.MERCHANT_ID AND b.MERCHANT_INFO_ID=c.MERCHANT_INFO_ID  and IS_EXECUTED='0' order by DEPOSIT_TIME DESC" + 
							" ) a where ROWNUM <= ?) where rnum >= ?";
		 	}
			...
			<td bgcolor="#CCCCCC"><div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><%= Integer.toString(no++)%></font></div></td>
            		<td bgcolor="#CCCCCC"><div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><%= rs.getString("d.SETTLE_DATE")%></font></div></td>
            		...
            		<td bgcolor="#CCCCCC"><div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><%= rs.getString("a.DOC_NUMBER")%></font></div></td>
                        <td bgcolor="#CCCCCC"><div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><%= rs.getString("c.BANK_NAME")%></font></div></td>
