...
<stripes:layout-render name="/web-starter/layout/standard.jsp" title="Payment Terminal List">
...
<%@page import="com.telkomsel.itvas.webstarter.User"%>
	...
	%>
        ...
        <table width="21%" border="0" cellspacing="0" cellpadding="0">
          <form name="form" method="post" action="list_payment.jsp">
            <tr bgcolor="#CC6633"> 
...
%>		
        <table width="80%" border="1" cellspacing="0" cellpadding="0" bordercolor="#FFF6EF">
          ...
          <tr> 
            ...
            <td bgcolor="#CC6633"> 
              <div align="center"><font color="#FFFFFF" size="1" face="Verdana, Arial, Helvetica, sans-serif"><strong>Msisdn</strong></font></div>
            </td>
            <td bgcolor="#CC6633"> 
          ...
          <%
// paging stuff
String sql = "select count(*) as jumlah from merchant m,merchant_info mi,reader_terminal rt where m.merchant_info_id = mi.merchant_info_id and m.merchant_id = rt.merchant_id and LOWER(name) like '%"+name.toLowerCase()+"%'";
...
// end of paging stuff
		 	...
		 	sql = "select * FROM (select a.*, ROWNUM rnum from (" + 
					 	"select KEYTERMINAL,name,rt.MSISDN,rt.DESCRIPTION,TERMINAL_ID,rt.ADDRESS,rt.TERMINAL_TYPE,rt.URL,rt.CHARGE_INFO from merchant m,merchant_info mi,reader_terminal rt where m.merchant_info_id = mi.merchant_info_id and m.merchant_id = rt.merchant_id and LOWER(name) like '%"+name.toLowerCase()+"%'" +
          ...
          <tr> 
            ...
            <td bgcolor="#EEEEEE"> 
              <div align="center"><font color="black" size="1" face="Verdana, Arial, Helvetica, sans-serif"><%= rs.getString("msisdn")%></font></div>
            </td>
            <td bgcolor="#EEEEEE"> 
            ...
            <td bgcolor="#EEEEEE"> 
              <div align="center"><font color="black" size="1" face="Verdana, Arial, Helvetica, sans-serif"><a href="edit_payment.jsp?stat=0&tid=<%= rs.getString("terminal_id")%>&name=<%= rs.getString("name")%>&msisdn=<%= rs.getString("msisdn")%>&desc=<%= rs.getString("description")%>&address=<%= rs.getString("address")%>&keyterminal=<%= (rs.getString("keyterminal")==null)?"":java.net.URLEncoder.encode(rs.getString("keyterminal"), "UTF-8")%>&terminaltype=<%= rs.getString("terminal_type")%>&lacci=<%= rs.getString("url")%>&clustername=<%= rs.getString("charge_info")%>" class="link">edit</a></font></div>
            </td>
            <td bgcolor="#EEEEEE"> 
              <div align="center"><font color="black" size="1" face="Verdana, Arial, Helvetica, sans-serif"><a href="delete_payment_terminal.jsp?tid=<%= rs.getString("terminal_id")%>" class="link" onClick="return checkDelete();">delete</a></font></div>
            </td>
          </tr>
          <%
			}
